...
<br>
<div><h1 align="center"><i>Pending Four Wheelers</i></h1></div><br>
<?php
	include 'db.php';
  ...
  <table class="table">
		<tr><th>Track ID</th><th>Name</th><th>Roll No</th><th>Department</th><th>Car Model</th><th>Car No</th><th>Driving License</th><th>RC Book</th><th>Insurance</th><th>ID Card</th><th>Approve/Reject</th></tr>
	<?php
...
if($desig=="Faculty")
{
	$sql="SELECT * FROM car WHERE verify='0' AND department='$depart' ORDER BY track_id";
			...
			?><tr><td><?php echo $row['track_id']; ?></td>
				 ...
				 <td><a href="Files/Id_Card/<?php echo $row['id_card'];?>" download>ID CARD</td>
				  <td><form action="qrcode.php" method="POST"><input type="hidden" value="<?php echo $row['track_id']; ?>" name="tid"><input type="hidden" value="<?php echo $d ?>" name="date"><input type="submit" class="btn btn-success" value="Approve"></form>
				  	<form action="deletecar.php" method="POST"><input type="hidden" value="<?php echo $row['track_id']; ?>" name="tid">
	...
	{
		$sql="SELECT * FROM car WHERE verify='1' AND department='$depart' ORDER BY track_id";
	if($res=mysqli_query($con,$sql))
	{
		while($row=mysqli_fetch_array($res))
		{
...
{
	$sql="SELECT * FROM car WHERE verify='2'  ORDER BY track_id";
	if($res=mysqli_query($con,$sql))
	{
		while($row=mysqli_fetch_array($res))
		{
